...
export function orderConfirmedToCustomer(order: Order, referralCode?: string, storeUrl?: string): string {
  ...
  const referralLine = referralCode
    ? `\n\n🎁 *Refer a friend & earn store credit!*\nShare your link: ${storeUrl ?? 'https://skctreats.in'}?ref=${referralCode}`
    : '';
  return `🙏 *Hare Krishna!* 🪷
...
export const deliveredMessage = (order: Order) => deliveredToCustomer(order, `https://skctreats.in/feedback/${order.id}`);
