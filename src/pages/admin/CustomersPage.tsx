            ...
            <div key={c.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              {expanded === c.id && (
                <div className="border-t border-gray-100 bg-gray-50 p-4 space-y-3">
                  ...
                  {(() => {
                    const pendingOrders = (customerOrders[c.id] || []).filter(o => o.paymentStatus === 'pending');
                    const livePending = pendingOrders.reduce((s, o) => s + (o.total || 0), 0);
                    const hasPending = livePending > 0;
                    const pctDraft = discountEdit[c.id] ?? '';
                    const pctVal = pctDraft === '' ? 0 : Math.min(100, Math.max(0, Number(pctDraft)));
                    const discountAmt = Math.round(livePending * pctVal / 100);
                    const amountAfter = livePending - discountAmt;
